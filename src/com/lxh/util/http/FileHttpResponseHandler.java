...
public class FileHttpResponseHandler extends LXH_HttpResponseHandler
{
	...
	protected void sendResponseMessage(HttpResponse response)
	{
		...
		try
		{
			...
			// -1鐨勮В鍐虫柟寮廋ontentLength 鍦ㄦ墜鏈鸿闂殑鏃跺�欏嚭鐜颁簡闂锛岃繑鍥炰负-1
			if (contentLenght == -1)
			{
			...
			LXH_Logger.v(TAG, "totalSize: " + totalSize);
			...
			} else if (tempFile.exists())
			{
				...
				// +
				// "-");
				LXH_Logger.v(TAG, "yahooo: "
						+ response.getEntity().getContentLength());
				previousFileSize = tempFile.length();

				LXH_Logger.v(TAG, "File is not complete, download now.");
				LXH_Logger.v(TAG, "File length:" + tempFile.length()
						+ " totalSize:" + totalSize);
			...
			LXH_Logger.v(TAG, "Download completed successfully.");
			result = bytesCopied;
		...
		if (result == -1 || interrupt || error != null)
		{
			if (error != null)
			{
				...
				{
					sendSuccessMessage(statusCode, "涓嬭浇鎴愬姛锛�".getBytes());
	...
	public int copy(InputStream input, RandomAccessFile out) throws IOException
	{
		...
		LXH_Logger.v(TAG, "length" + out.length());
		int count = 0, n = 0;
