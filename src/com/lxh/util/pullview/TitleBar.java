...
/**
 * © 2012 amsoft.cn 名称：BottomBar.java 描述：标题栏实现.
 * 
 * @author 还如一梦中
 * @version v1.0
...
public class TitleBar extends LinearLayout {
	...
	/** 左侧的View */
	protected LinearLayout leftView = null;
	/** 左侧的Logo图标View. */
	...
	public void ininTitleBar(Context context) {
		...
		android.view.ViewGroup.LayoutParams pa = logoView.getLayoutParams();
		if (h != 0 && h > 0) {
			if(pa!=null){
			pa.height = h;
			logoView.setLayoutParams(pa);
			}
		} else if (w != 0 && w > 0) {
			if(pa!=null){
			pa.width = w;
		...
		//加左边的布局
				...
				leftView.setHorizontalGravity(Gravity.LEFT);
				leftView.setGravity(Gravity.CENTER_VERTICAL);
				leftView.setVisibility(View.GONE);
				this.addView(leftView, layoutParamsWW);
		...
		this.setLayoutParams(new LayoutParams(20, 20));
	}

	private int h = 0;
	private int w = 0;

	public void setlogoViewH(int h) {
		this.h = h;
	}

	public void setlogoViewW(int w) {
		this.w = w;
	}
	...
	public void setTitleTextSize(int titleTextSize) {
		this.titleTextBtn.setTextSize(px2sp(context,titleTextSize));
	}
	/**
     * 将px值转换为sp值，保证文字大小不变
     * 
	...
	public void setTitleBarGravity(int gravity1, int gravity2) {
		...
		if ((gravity1 == Gravity.CENTER_HORIZONTAL || gravity1 == Gravity.CENTER)) {
			if (leftWidth == 0 && rightWidth == 0 && leftViewwidth == 0) {
	 ...
	 *            右边布局对齐方式
	 */
	public void setTitleBarGravity(int gravity1, int gravity2,int gravity3) {
		AbViewUtil.measureView(this.rightLayout);
		AbViewUtil.measureView(this.logoView);
		AbViewUtil.measureView(this.leftView);
		int leftWidth = this.logoView.getMeasuredWidth();
		...
		if ((gravity1 == Gravity.CENTER_HORIZONTAL || gravity1 == Gravity.CENTER)) {
			if (leftWidth == 0 && rightWidth == 0 && leftViewwidth == 0) {
				...
				if (gravity2 == Gravity.CENTER|| gravity2 == Gravity.CENTER_HORIZONTAL&& gravity3 !=Gravity.LEFT) {
					this.titleTextLayout.setGravity(Gravity.CENTER_HORIZONTAL);
					this.rightLayout.setHorizontalGravity(Gravity.LEFT);
					this.leftView.setHorizontalGravity(Gravity.LEFT);
					this.titleTextBtn.setGravity(Gravity.CENTER);
					int offset = leftViewwidth - rightWidth;
					if (offset > 0) {
						this.titleTextLayoutParams.rightMargin = offset;
					} else {
						this.titleTextLayoutParams.leftMargin = Math
	 ...
	 * @return
	 */
	public LinearLayout getLogoLayoutView() {
		return leftView;
	}
	...
	public void setleftView(Drawable drawable) {
		...
		leftView.setBackgroundDrawable(drawable);
	}
	/**
	 * 描述：设置Logo的背景资源.
	 * 
	...
	public void setleftView(int resId) {
		leftView.setVisibility(View.VISIBLE);
		leftView.addView(mInflater.inflate(resId, null), layoutParamsWW);
	}
