...
import com.lxh.util.db.database.CursorHelper;
import com.lxh.util.db.database.DaoConfig;
import com.lxh.util.db.database.DbModel;
import com.lxh.util.db.database.OneToManyLazyLoader;
import com.lxh.util.db.database.SqlBuilder;
import com.lxh.util.db.database.SqlInfo;
import com.lxh.util.db.database.utils.KeyValue;
import com.lxh.util.db.database.utils.ManyToOne;
import com.lxh.util.db.database.utils.OneToMany;
import com.lxh.util.db.database.utils.TableInfo;
...
public class DB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, DB> daoMap = new HashMap<String, DB>();
    ...
    private synchronized static DB getInstance(DaoConfig daoConfig) {
        DB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new DB(daoConfig);
     ...
     */
    public static DB create(boolean isDebug) {
        Context cxt =LXH_Application.getApplication();
        if (cxt == null) {
     ...
     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static DB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static DB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static DB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static DB create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static DB create(DaoConfig daoConfig) {
