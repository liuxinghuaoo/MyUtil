...
public class AbPullToRefreshView extends LinearLayout {
	...
	private AbListViewAlert abListViewAlert;
	...
	public void setHasOther(boolean isHasOther) {
				...
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					...
					if (view.getLastVisiblePosition() == view.getCount() - 1) {
						...
						if (mOnFooterLoadListener != null) {
							mOnFooterLoadListener
									.onFooterLoad(AbPullToRefreshView.this);
						}
					}
				}
			}
	...
	private void addAlertView(AttributeSet attrs) {
		abListViewAlert = new AbListViewAlert(mContext,
				mOnHeaderRefreshListener);

		TypedArray typedArray = this.getContext().obtainStyledAttributes(attrs,
				R.styleable.AbPullToRefreshView);
		...
		abListViewAlert.setVisibility(View.GONE);
		addView(abListViewAlert, params);
	}
	 ...
	 * @param vis
	 */
	public void onAlertViewVisibility(boolean vis) {
		if (abListViewAlert != null)
			if (vis) {
				abListViewAlert.setVisibility(View.VISIBLE);
			} else {
				abListViewAlert.setVisibility(View.GONE);
			}
	}
	 ...
	 *            文字
	 */
	public void setAlertView(int resId, String alertText) {
		if (abListViewAlert != null) {
			abListViewAlert.getImageView().setImageResource(resId);
			abListViewAlert.getTextView().setText(alertText);
			abListViewAlert
					.setOnHeaderRefreshListener(mOnHeaderRefreshListener);
		}
	}
	 ...
	 * @param alertTvHeight
	 */
	public void setAlertTvWidthOrHeight(int alertTvWidth, int alertTvHeight) {
		abListViewAlert.getTextView().setWidth(alertTvWidth);
		abListViewAlert.getTextView().setHeight(alertTvHeight);
	}
	 ...
	 * @param tvColor
	 */
	public void setAlertColor(int tvColor) {
		abListViewAlert.getTextView().setTextColor(tvColor);
	}
	 ...
	 * @param tvSize
	 */
	public void setAlertTextSize(float tvSize) {
		abListViewAlert.getTextView().setTextSize(
				DisplayUtil.px2sp(mContext,
						DisplayUtil.dip2px(mContext, tvSize)));
	}
	...
	public boolean isRefreshOrLoad() {
		return isRefreshOrLoad;
	}

	public void setRefreshOrLoad(boolean isRefreshOrLoad) {
		this.isRefreshOrLoad = isRefreshOrLoad;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			break;
		case MotionEvent.ACTION_MOVE:
			MyLog.e(TAG, "isHasOther = " + isHasOther);
			if (isHasOther)
				return true;
			int deltaY = y - mLastMotionY;
	...
	public void setOnMneu(OnMenu menu) {
		onmenu = menu;
	}
	...
	public interface OnMenu {
		public void OnMenuCheckListener();
	}
